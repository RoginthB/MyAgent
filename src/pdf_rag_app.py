...
def process_pdf(uploaded_file):
    ...
    try:
        ...
        # Initialize PDF RAG if not already done
        if st.session_state.pdf_rag is None:
            st.session_state.pdf_rag = PDFRAG()
        
        # Index the PDF content
        with st.spinner('🔄 Indexing PDF content... This may take a moment.'):
            st.session_state.pdf_rag.index_pdf_content(tmp_path)
...
def get_agent_response(query: str) -> tuple:
    ...
    if 'pdf_rag' not in st.session_state or st.session_state.pdf_rag is None:
        yield ("⚠️ Error: PDF RAG system is not initialized. Please reload the page and upload a PDF.", [])
        return

    # Capture the instance locally to avoid session_state access issues inside the tool
    pdf_rag_instance = st.session_state.pdf_rag
    sources_info = []
    ...
    def retrieve_content(query):
        ...
        retrieve_docs = pdf_rag_instance.vector_store.similarity_search_with_score(query=query, k=2)
    ...
    st.markdown("""
        <div class="info-box">
            <p style="margin: 0; font-size: 16px;">
                <strong>Welcome to PDF RAG Assistant!</strong><br>
    ...
    st.markdown("""
        <div style="padding: 15px; background-color: rgba(255, 255, 255, 0.05); border-radius: 10px;">
            ...
            <ol style="font-size: 14px;">
                ...
                <li>Click "Process PDF" to index</li>
                ...
                with st.expander("📑 View Sources", expanded=False):
                    ...
                    for idx, source in enumerate(message["sources"], 1):
                        ...
                        <div style="background: linear-gradient(135deg, rgba(102, 126, 234, 0.1) 0%, rgba(118, 75, 162, 0.1) 100%); 
                            ...
                            📊 <strong>Relevance:</strong> {source['relevance']}<br>
                            <details>
                                <summary style="cursor: pointer; color: #667eea; font-weight: 600; margin-top: 10px;">View Content Preview</summary>
                                <div style="margin-top: 10px; padding: 10px; background: rgba(255, 255, 255, 0.05); border-radius: 5px; font-size: 14px;">
                                    {source['content']}
                                </div>
                            </details>
                        </div>
    ...
    st.markdown("""
        <div style="text-align: center; padding: 50px 20px;">
            ...
            <p style="font-size: 18px; color: #666;">
                Upload a PDF document using the sidebar to begin your intelligent document analysis journey.
            </p>
